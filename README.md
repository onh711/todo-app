...
- **リファクタリング**：冗長な書き方になってしまっている個所のリファクタリング
- **全ページのレスポンシブ対応**：現在はダッシュボードページのみ
- **バリデーション強化**：フロントエンド側でのバリデーションを実装できていないページがある
- **統計機能**：赤ちゃんの行動の履歴を統計として表示する(平均睡眠時間、泣く時間帯の傾向等)
- **成長記録機能**：赤ちゃんの成長記録を記録できる機能(身長、体重等)
- **赤ちゃん複数登録**：現在は一人の赤ちゃんのみの登録のため複数名登録できるようにしたい
...
カレンダービューで登録したい時間帯をクリック → アクション（寝る、授乳など）を選択して記録
